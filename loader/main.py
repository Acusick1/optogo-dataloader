...
from shared.sql import database
    ...
    with Session(database.engine) as session:
        ...
        for journey in trip.journeys:
            ...
            if flights is not None:
                ...
                post_journey_flight(
                    journey_id=journey_db.id,
                    flight_dbs=flight_dbs,
                    session=session
                )
...
def post_journey_flight(journey_id: int, flight_dbs: list[models.Flight], session: Session):
    ...
    for flight_db in flight_dbs:
        ...
        _ = database.get_or_add(session, models.JourneyFlight, **association_schema.dict())


def post_request_journey(request_id: int, journey_dbs: list[models.Journey], price, session: Session):

    journey_id_1 = journey_dbs[0].id
    journey_id_2 = journey_dbs[1].id if len(journey_dbs) > 1 else None

    # Filter by unique columns
    association = session.query(models.RequestJourney).filter_by(
        request_id=request_id,
